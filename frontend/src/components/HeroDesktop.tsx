...
const HeroDesktop = ({ onTerminalOpen }: HeroDesktopProps) => {
  ...
  return (
    <section
      id="home"
      className="min-h-screen relative overflow-hidden flex flex-col text-foreground bg-cover bg-center bg-no-repeat"
      style={{ backgroundImage: "url('/f1g5Ap.png')" }}
    >
      {/* Subtle gradient overlay */}
      <div
        className="absolute inset-0 pointer-events-none"
        style={{
          background: "linear-gradient(180deg, hsl(0 0% 0% / 0.18), hsl(270 45% 5% / 0.42))",
        }}
      />
      {/* Top Status Bar */}
      <header className="sticky top-0 z-50 w-full bg-retro-bar text-retro-ink flex items-center justify-between px-6 py-3 border-b-4 border-retro-border">
        <div className="flex items-center gap-3">
          <div className="w-8 h-8 rounded-full bg-retro-border flex items-center justify-center text-retro-bar">☺</div>
          <span className="text-lg font-pixel tracking-wide">Mishti's Desktop</span>
        </div>
      ...
      {/* Desktop Workspace */}
      <div className="flex-1 w-full px-4 md:px-8 py-8 relative z-10">
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 max-w-6xl mx-auto">
          {/* Desktop Icons floating on background */}
          <section className="lg:col-span-8 relative min-h-[420px]">
                <div className="inline-flex items-center gap-3 bg-retro-panel/90 border border-retro-border px-4 py-2 font-pixel text-sm text-retro-ink rounded-b-lg shadow-retro-icon">
                  Desktop
                </div>
                <div className="absolute top-4 right-0 bg-retro-panel/70 border border-retro-border px-3 py-2 text-xs text-retro-ink rounded-l-lg">
                ...
                >
                  <div className="flex items-center justify-center group-hover:-translate-y-1 transition-transform">
                    ...
                    />
                      ...
                      <span className="font-pixel text-xs text-retro-ink group-hover:text-neon-cyan transition-colors">
                        {app.label}
                      </span>
                </button>
              ))}
          ...
          <aside className="lg:col-span-4 bg-retro-window border-4 border-retro-border shadow-retro flex flex-col">
                <div className="flex items-center justify-between bg-retro-panel border-b-2 border-retro-border px-4 py-2">
                  <span className="font-pixel text-sm text-retro-ink">StudioCam</span>
            ...
            <div className="flex-1 p-4 flex flex-col gap-4">
              <div className=" bg-card/60 border border-retro-border p-3 flex items-center gap-3 rounded">
                ...
                <div>
                  ...
                  <p className="text-xs text-muted-foreground">CS Student · ML Engineer</p>
                </div>
              </div>
